Set-Content -Path "src\pages\JobDetail.tsx" -Encoding UTF8 -Value @'
...
function ReviewModal({ jobId, providerId, onDone }: { jobId: string; providerId: string; onDone: () => void }) {
  ...
  async function submit() {
    ...
    setLoading(false);
    if (error) { toast({ title: "Error al enviar reseña", variant: "destructive" }); return; }
  ...
  return (
    <div className="bg-white border border-gray-100 rounded-2xl p-5">
      <h3 className="font-semibold text-gray-900 mb-4 flex items-center gap-2">
        <Star className="h-4 w-4 text-amber-400" /> Dejar una reseña
      </h3>
...
Write-Host "✅ Chat rediseñado!" -ForegroundColor Green
```