...
import {
  ...
  Wallet, ShieldCheck, ExternalLink, LinkIcon
} from "lucide-react";
...
type Section = "personal" | "provider" | "prices" | "photos" | "availability" | "payments";
...
export default function Settings() {
  ...
  // Manejar retorno desde el OAuth callback
  useEffect(() => {
    const connected = searchParams.get("mp_connected");
    const mpError = searchParams.get("mp_error");
    if (connected === "true") {
      toast({ title: "MercadoPago conectado", description: "Ya podés recibir cobros" });
      refreshUser();
      setOpenSection("payments");
    } else if (mpError) {
      toast({
        title: "No se pudo conectar MercadoPago",
        description: MP_ERROR_MESSAGES[mpError] ?? mpError,
        variant: "destructive",
      });
      setOpenSection("payments");
    }
  ...
  async function handleMpConnect() {
    ...
    const { url, error } = await startMpConnect();
    if (error || !url) {
      toast({ title: "Error al iniciar conexión", description: error, variant: "destructive" });
  ...
  async function handleMpDisconnect() {
    if (!provider) return;
    if (!confirm("¿Seguro que querés desconectar MercadoPago? No vas a poder recibir cobros hasta volver a conectarte.")) return;
    setMpDisconnecting(true);
    const { error } = await disconnectMp(provider.id);
    setMpDisconnecting(false);
    if (error) {
      toast({ title: "Error al desconectar", description: error, variant: "destructive" });
      return;
    }
  ...
  const [form, setForm] = useState({
    ...
    price_list: (provider?.price_list ?? []).map(p => ({ ...p, price: String(p.price) })) as PriceItem[],
  });
  ...
  async function handlePhotoUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const path = `work/${user.id}/${Date.now()}_${file.name}`;
  ...
  // Subida del avatar (foto de perfil del usuario). Se guarda al instante.
  async function handleAvatarUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (uploaded) {
      ...
      // Persistimos el avatar de inmediato para que se vea en toda la app
  ...
  async function save() {
    ...
    await supabase.from("users").update({
      name: form.name, phone: form.phone, city: form.city, avatar_url: form.avatar_url || null
    }).eq("id", user!.id);
      ...
      <header className="sticky top-0 z-50 bg-white border-b border-gray-100">
        <div className="max-w-lg mx-auto px-4 flex items-center gap-3 h-16">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400">Editá tu información</p>
          </div>
        </div>
      </header>
      ...
      <div className="max-w-lg mx-auto px-4 py-6 space-y-3">
          ...
          <div className="bg-white border border-gray-100 rounded-2xl p-5">
            ...
            <div className="grid grid-cols-3 gap-2 mt-3">
              {[
                ...
                { label: "Categorías", done: form.categories.length > 0 },
            ...
            <div className="px-5 pb-5 space-y-4 border-t border-gray-50">
              ...
              <div className="flex items-center gap-4 pt-4">
                <div className="relative">
                  {form.avatar_url ? (
                    <img src={form.avatar_url} alt="Foto de perfil"
                      className="h-20 w-20 rounded-2xl object-cover border border-gray-100" />
                  ) : (
                    <div className="h-20 w-20 rounded-2xl bg-green-100 flex items-center justify-center text-2xl font-bold text-green-700">
                      {(form.name || "U").charAt(0).toUpperCase()}
                    </div>
                  )}
                  <button type="button" onClick={() => avatarInputRef.current?.click()}
                    className="absolute -bottom-1 -right-1 h-7 w-7 bg-green-600 rounded-xl flex items-center justify-center text-white shadow-sm hover:bg-green-700 transition-colors">
                    {uploadingAvatar ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Camera className="h-3.5 w-3.5" />}
                  </button>
                </div>
                <div>
                  <div className="font-semibold text-sm text-gray-900">Foto de perfil</div>
                  <div className="text-xs text-gray-400">Una buena foto genera más confianza. JPG o PNG.</div>
                </div>
              ...
              <div className="space-y-3">
                {[
                  ...
                  { label: "Teléfono", key: "phone", placeholder: "+54 9 11 1234-5678", type: "tel" },
        ...
        {provider && (
          <>
            ...
            {/* Payments / MercadoPago — oculto hasta que MP apruebe Marketplace para nuestra app.
                Para reactivar: cambiar SHOW_MP_OAUTH a true acá y MP_USE_MARKETPLACE a "true"
                en los secrets de Supabase. */}
            {/* @ts-ignore false flag para activar cuando MP apruebe */}
            {false && (
            <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden">
              <SectionHeader id="payments" title="Cobros con MercadoPago" icon={Wallet} />
                  ...
                  {provider.mp_user_id ? (
                    <div className="space-y-3">
                      <div className="flex items-center gap-3 p-4 bg-green-50 border border-green-100 rounded-xl">
                        ...
                        <div className="flex-1">
                          <div className="font-semibold text-sm text-green-900">Cuenta conectada</div>
                          <div className="text-xs text-green-700">
                            ID MercadoPago: <span className="font-mono">{provider.mp_user_id}</span>
                          </div>
                          {provider.mp_connected_at && (
                            <div className="text-xs text-green-600 mt-0.5">
                              Conectada desde {new Date(provider.mp_connected_at).toLocaleDateString("es-AR")}
                            </div>
                          )}
                        </div>
                      </div>
                      <p className="text-xs text-gray-500">
                        Los pagos de tus trabajos se acreditan directo en tu cuenta de MercadoPago,
                        descontada la comisión de ServiMarket.
                      </p>
                      <button onClick={handleMpDisconnect} disabled={mpDisconnecting}
                        className="w-full h-10 border border-red-200 text-red-500 rounded-xl text-sm font-medium hover:bg-red-50 transition-colors flex items-center justify-center gap-2 disabled:opacity-50">
                        {mpDisconnecting ? <Loader2 className="h-4 w-4 animate-spin" /> : <X className="h-4 w-4" />}
                        Desconectar cuenta
                      </button>
                    </div>
                  ) : (
                    <div className="space-y-3">
                      <div className="p-4 bg-amber-50 border border-amber-100 rounded-xl">
                        <div className="font-semibold text-sm text-amber-900 mb-1">Conectá tu cuenta de MercadoPago</div>
                        <div className="text-xs text-amber-700">
                          Sin la conexión, los clientes no van a poder pagarte por la app.
                          Es gratis y rápido — necesitás una cuenta de MercadoPago a tu nombre.
                        </div>
                      </div>
                      <ul className="text-xs text-gray-600 space-y-1.5 px-1">
                        <li className="flex items-start gap-2">
                          <CheckCircle2 className="h-3.5 w-3.5 text-green-600 shrink-0 mt-0.5" />
                          Los pagos se acreditan automáticamente en tu cuenta
                        </li>
                        <li className="flex items-start gap-2">
                          <CheckCircle2 className="h-3.5 w-3.5 text-green-600 shrink-0 mt-0.5" />
                          La comisión de ServiMarket se descuenta sola, sin papeleo
                        </li>
                        <li className="flex items-start gap-2">
                          <CheckCircle2 className="h-3.5 w-3.5 text-green-600 shrink-0 mt-0.5" />
                          Podés desconectar la cuenta cuando quieras
                        </li>
                      </ul>
                      <button onClick={handleMpConnect} disabled={mpConnecting}
                        className="w-full h-12 bg-[#009ee3] text-white rounded-xl font-semibold flex items-center justify-center gap-2 hover:bg-[#0084c4] transition-colors disabled:opacity-50">
                        {mpConnecting
                          ? <Loader2 className="h-5 w-5 animate-spin" />
                          : <><LinkIcon className="h-5 w-5" /> Conectar con MercadoPago <ExternalLink className="h-4 w-4" /></>}
                      </button>
                    </div>
                  )}
                </div>
